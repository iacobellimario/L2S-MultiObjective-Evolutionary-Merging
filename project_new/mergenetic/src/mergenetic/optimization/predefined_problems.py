...
from mergenetic.evaluation.qwen_evaluator import QwenEvaluator
...
@dataclass
class ConfigQwen:
    qwen_datasets: list = field(default_factory=lambda: ["math"])
    qwen_max_samples: int = 20
    qwen_split: str = None
    qwen_seed: int = 0
    qwen_output_root: str = None

class L2SMathReasoningProblem(MergingProblem): 
    def __init__(self, merger, search_df, test_df, lang_id, conf_pe, lm_eval_tasks=None,
                 n_var=11, n_obj=2, n_eq_constr=0, n_ieq_constr=0, xl=0, xu=1, eval_batch_size=5,
    ...
    def metrics_4_genotype(self, model, tokenizer=None):
        datasets = getattr(self.conf_pe, "qwen_datasets",["math"])
        split = getattr(self.conf_pe, "qwen_split", "train")
        max_samples = getattr(self.conf_pe, "qwen_max_samples", 50)
        output_root = getattr(self.conf_pe, "qwen_output_root")
        seed = getattr(self.conf_pe, "qwen_seed", 42)
        ...
        if isinstance(model, (str, Path)):
            ...
            model_path = getattr(model, "name_or_path", None) or getattr(model, "hf_id", None) or None
        if model_path is None:
            raise ValueError("Qwen evaluator requires model path (string). Pass model path or ensure load_model() returns a path when use_lm_eval=True.")
       
        evaluator.evaluate(model_path)
        df_eval = evaluator.get_data()
           
        acc = df_eval["correctness"].mean() if (not df_eval.empty and "correctness" in df_eval.columns) else 0.0
        f1 = -acc
        f2 = df_eval["answer_length"].mean() if (not df_eval.empty and "answer_length" in df_eval.columns) else math.inf
        
        print(f"Fitness: acc={acc}, len={f2}")
        
        # [f1, f2] for multi-objective optimization
        # [f1] or [f2] for single-objectie optimization
        return [f1, f2], f"Fitness: acc={acc:.2f}, len={f2:.2f}"
    ...
    def test(self, genotype, base_model: Optional[Path] = None):
        ...
        tokenizer = (None if self.use_lm_eval else tokenizer) or self.tokenizer
        return self.metrics_4_genotype(model if self.use_lm_eval else model, tokenizer)