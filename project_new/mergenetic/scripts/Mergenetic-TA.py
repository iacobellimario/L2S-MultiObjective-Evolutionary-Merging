...
config.bench = None
config.seed = SEED
config.run_id = "Mergenetic-TA-7B-MATH-Random"
config.device = "auto" 
...
est_parameters = ConfigQwen(
    qwen_datasets = ["math"],
    # use "test" split for RANDOM SAMPLING
    # use "entropy-test-*B" for ENTROPY (check /Qwen2.5-Math/evaluation/data/math)
    qwen_split = "test", 
    qwen_max_samples = 50,
    qwen_seed = SEED,
...
# ========== Problem ==========
tokenizer_path = os.path.join(model_dir, "DeepSeek-R1-Distill-Qwen-7B") # tokenizer for length computation
...
problem = L2SMathReasoningProblem(
    ...
    n_var=1, # w for TA
    n_obj=2, # (-accuracy, length) --> set to 1 for single-objective (check /mergenetic/src/mergenetic/optimization/predefined_problems.py)
    n_eq_constr=0,
    n_ieq_constr=0,
    discrete=True,
